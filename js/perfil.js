document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (btnAbrirEdit) {
        btnAbrirEdit.addEventListener('click', async () => {
            ...
            if (usuario) {
                document.getElementById('edit-telefono').value = usuario.telefono_contacto || '';
    ...
    if (formEdit) {
        formEdit.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                alert("¡Datos actualizados correctamente! Si actualizaste el correo, revisa tu bandeja para confirmar el cambio.");
...
async function calcularNivelCliente(supabase, userId) {
    const META_FRECUENTE = 300; //Meta para ser frecuente
    ...
    try {
        ...
        if (totalGastado >= META_FRECUENTE) {
            ...
            await supabase.from('perfiles_cliente').update({ es_frecuente: true }).eq('id_cliente', userId);
...
async function cargarHistorialPedidosBD(supabase, userId) {
    ...
    try {
        const { data: pedidos, error } = await supabase
            .from('pedidos')
            .select(`
                id_pedido, fecha_pedido, estado_pedido, total_factura,
                detalles_pedido (
        ...
        pedidos.forEach(pedido => {
            ...
            const html = `
                <div class="order-item">
                    ...
                    <button class="btn-view-details" onclick="verDetallePedidoBD('${pedido.id_pedido.slice(-6)}', '${fechaStr}', '${pedido.total_factura}', '${productosEncoded}')">
...
window.verDetallePedidoBD = function (id, fecha, total, encoded) {
    ...
    body.innerHTML = `<h3>Pedido #${id}</h3><p>${fecha}</p><ul>${list}</ul><h3 style="text-align:right; color:#E6B325; background-color: transparent;">Total: $${parseFloat(total).toFixed(2)}</h3>`;
